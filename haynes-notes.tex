...
Now the induced map $Z^{(n)}=\frac{Z^n}{F_{n-1}Z^n}\to\frac{E\pi \times_\pi Z^n}{E\pi \times_\pi F_{n-1}Z^n} = D_\pi Z$ is exactly $i_Z$, so we can study $i^*_Z$ using the associated morphism of relative Serre spectral sequences.

The relative Serre spectral sequence for the pair on the left has untwisted coefficients:
\[
_LE_2^{*, *} = H^*(*; H^*(Z^n, F_{n-1} Z^n)) \Rightarrow \widetilde H^*(Z^{(n)})
.\]
...
\[
_RE_2^{*, *} = H^*(B\pi; \{H^*(Z^n, F_{n-1} Z^n)\}) \Rightarrow \widetilde H^*(D_\pi Z)
.\]
