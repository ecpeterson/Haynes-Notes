...
\end{lem}
\noindent For example, $\widetilde K S^2 = \Z \langle \bundle{L} - 1 \rangle$ is subject to the relation $(\bundle{L} - 1)^2 = 0$.  So $K(S^2) = \Z[x]/x^2$.  While we're at it, let's compute the Adams operations.  $\bundle{L}$ is a line bundle, so $\psi^k \bundle{L} = \bundle{L}^k$.  Thus
\[\psi^k(x)  = \psi^k(\bundle{L} - 1) = \bundle{L}^k - 1=(1+x)^k-1=kx.\]
...
\end{claim}
\begin{proof}
Writing $\nu(n)$ for the largest number $r$ such that $2^r$ divides $n$, so that $2^n|3^n-1$ iff $\nu(3^n-1)\geq n$. If one can prove the following formula, the result is immediate:
\[
\nu(3^n-1) = \begin{cases} 1 & \hbox{$n$ odd}, \\ \nu(n) + 2 & \hbox{$n$ even}.\end{cases}
\]
To make this calculation, we have the following argument, due to David Anick. Write $k = \nu( n)$, and $n = 2^k d$ where $d$ is odd.  If $k = 0$, write $n = 2j + 1$, and $3^{2j+1} - 1 = 9^j \cdot 3 - 1 \equiv 2 \pmod 8$, so $2$ divides $3^\alpha - 1$ once.  Note also $3^{2k+1}+1 = 9^j \cdot 3 + 1 \equiv 4 \pmod 8$, so $2$ divides this twice.  For $k > 0$, note $3^{2^k d} - 1 = (3^{2^{k-1}d} + 1)(3^{2^{k-1}d} - 1)$; continuing we get
\[
