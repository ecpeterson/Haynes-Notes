...
From what we had before, the attaching map $\pi:S^{n-2}\to\RP^{n-2}_{-N}$ pulls back stably to a map $\overline\pi:S^{n-2}\to\RP^{n-\rho(n)-1}_{-N}$, whose compression onto the top cell $S^{n-\rho(n)-1}$ is $j_{\nu(n)}\neq0$. Thus, applying $\pi_{n-2}^S$ to the above diagram, we have elements as in the box at the right. As $j_{\nu(n)}$ is nonzero, and the bottom row is exact ($\pi_*^S$ is a cohomology theory) there can be no element $?\in\pi_{n-2}^S(\RP^{n-\rho(n)-2}_{-N})$ mapping to $\overline \pi$.
\ResumeHere
\[\xymatrix{
&\pi_{n-2}^S(\RP^{n-2}_{-N})\ar[r]&\pi_{n-2}^S(\RP^{n-2}_{n-\rho(n)-1})\\
\pi_{n-2}^S(\RP^{n-\rho(n)-2}_{-N})\ar[r]&\pi_{n-2}^S(\RP^{n-\rho(n)-1}_{-N})\ar[r]\ar[u]&\pi_{n-2}^S(S^{n-\rho(n)-1})\ar[u]
}\xymatrix{
&\pi\ar@{|->}[r]&\pi\\
?\ar@{|..>}[r]&\overline\pi\ar@{|->}[r]\ar@{|->}[u]&\pi\ar@{|->}[u]
}\]
\begin{diagram}[height=2em]
& & \vdots & & \vdots \\
& & \uTo & & \uTo \\
