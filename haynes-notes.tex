...
\Bullet $nL\onto \R P^{k-1}$ is the normal bundle of this inclusion into $\R P^{n+k-1}$.

\Bullet $T(nL)\simeq \RP^{n+k-1}_n:=\R P^{n+k-1}/\R P^{n-1}$.

\Bullet In particular, if $S^{n-1}$ admits $k-1$ vector fields, then $T(n\epsilon)\simeq \R P^{n+k-1}_n$.
}
