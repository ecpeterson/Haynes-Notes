...
We seek to prove the following properties of the Adams operations, and will do so by constructing them from another perspective in the next sections:
...
We can show directly though that $\xi^k_n=\psi^k_n$, viewing each as a class function on $U(n)$. Suppose that in the above generating function, we substitute the $j^\textup{th}$ elementary symmetric polynomial in $n$ variables $z_1,\ldots,z_n$ for $\Lambda^i(E)$, for all $i$. Then the generating function becomes $\sum_{i=1}^\infty t^ip_i $, where $p_i=\sum_{j=1}^n z_j^i$ is $i^\textup{th}$ power sum symmetric polynomial.
To see this, note that by definition of the elementary symmetric polynomials the generating function becomes 
\[-t\frac{d}{dt}\log\prod_{j=1}^n(1-t z_j)=-t\sum_{j=1}^n\frac{-z_j}{1-tz_j}=t\sum_{j=1}^n \left(z_j+tz_j^2+t^2z_j^3+\cdots\right)=\sum_{k=1}^\infty t^k p_k.\]
This completes the proof, \textbf{which needs to be made a lot clearer}.
\end{proof}
...
\Bullet By understanding how to compose additive sequences, we prove $\psi^k\psi^l=\psi^{kl}$.
