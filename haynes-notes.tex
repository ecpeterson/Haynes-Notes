...
\end{claim}
\begin{proof}
Whether $n$ is even or odd, we know that the cohomology algebra of $E=\Omega S^{n+1}$ has a copy\footnote{Or a copy of $\Z_{(2)}$ when we are working at $(2)$. The distinction is unimportant and will be ignored} of $\Z$ in each degree which is a multiple of $n$. We also know that if $a\in H^{on}(E)$ and $b\in H^{en}(E)$ are nonzero, where $o$ is odd and $e$ is even, then $ab\neq0$. Finally, the cohomology algebra of $B=\Omega S^{2n+1}$ has a copy of $\Z$ in each degree which is a multiple of $2n$, and we showed above that $H^{2kn}(h_2)$ is an isomorphism for all $k$.

As the maps $H^{2kn}(h_2)$ are isomorphisms, no differentials can ever hit the entries $E^{2kn,0}$ of the associated Serre spectral sequence.\footnote{To understand this, consider the edge homomorphism.} Thus, no differentials out of the entries $E^{0,k}$ for $k<3n-1$ can be nonzero. In particular, all of these groups are zero except for $E_2^{0,n}$, which is a copy of $\Z$.
