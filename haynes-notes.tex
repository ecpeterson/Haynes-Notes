...
& \cong C_{k-8}^+(16).
\end{align*}
(Similarly, $C_k^- \cong C_{k-8}^-(16)$.)  Next, we can quickly determine the dimensions $a_k$.  The fourth column is $\phi_k = \log_2 a_k$, where $a_k$ is the dimension of the minimal representation.  From $C_k^+ \cong C_{k-8}^+(16)$ we get that $a_{k+8} = 16 a_k$, or $\phi(k + 8) = \phi(k) + 4$. % be consistent about whether phi should be a subscript or a function
...
On the other hand, we can recover $L$ as $S^{k-1} \times_{\Z/2} \R$; this is an example of the ``Borel construction.'' % define what a twisted product is
