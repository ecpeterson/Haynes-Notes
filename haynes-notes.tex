...
As an example of how things work, remember earlier we studied the group $J(X)$ which had to do with stable fiber homotopy equivalence.  Let $V\downarrow X$ be an $(n-1)$-sphere bundle.  In unit \ref{FactsAboutThomSpaces}, we saw that $V\downarrow X$ being fiber homotopy trivial implied a splitting
\begin{diagram}[height=2em]
T(V) & \rTo & S^n \\
\uTo & \ruTo_\simeq \\
S^n.
...
\end{itemize}
\end{cor}
\ConfusedBox{The whole function spectrum Spiel was omitted, and needs to be replaced.}
