...
We have a map of bundle-subbundle pairs:
\[\xymatrix{
F_{n-1}Z^n\ar@{^{(}->}[r]\ar[d]&Z^n\ar[d]&F_{n-1}Z^n\ar@{^{(}->}[r]\ar[d]&Z^n\ar[d]\\
F_{n-1}Z^n\ar@{^{(}->}[r]\ar[dr]&Z^n\ar[d]&
E\pi\times_\pi F_{n-1}Z^n\ar@{^{(}->}[r]\ar[dr]&E\pi\times_\pi Z^n\ar[d]\\
&{*}&&B\pi
}\]
Now the induced map $Z^{(n)}=Z^n/F_{n-1}Z^n\to\frac{E\pi \times_\pi Z^n}{E\pi \times_\pi F_{n-1}Z^n} = D_\pi Z$ is exactly $i_Z$, so we can study $i^*_Z$ using the associated morphism of relative serre spectral sequences.

The spectral sequence for the first pair looks like:
\[
E_2^{*, *} = H^*(*; H^*(Z^n, F_{n-1} Z^n)) \Rightarrow \widetilde H^*(Z^{(n)})
.\]
...
.\]
Now $H^*(Z^n, F_{n-1} Z^n) \cong \widetilde H^* Z^{(n)} \cong \widetilde H^*(Z)^{\otimes n}$, and this last isomorphism is equivariant with respect to permutations, a fact that is not trivial (and likely to be false with other cohomology theories).  It follows that in both spectral sequences everything is zero below the horizontal line at height $nq$ in the $E_2$ term.  So, % why?
\[
\widetilde H^{nq}(D_\pi Z) = H^0(B\pi; \{\widetilde H^q(Z)^{\otimes n}\}) = (\widetilde H^q(Z)^{\otimes n})^\pi
.\qedhere\]
Moreover, the map $i^*_Z:\widetilde H^{nq}(D_\pi Z)\to \widetilde H^{nq}(Z^{(n)})$ is induced by the corresponding map on the $E_2$ page. This is the map 
...
So $\widetilde H^{nq}D_\pi K_q= \Z/p$.  Moreover, as $\pi$ can only act trivially on $\widetilde H^{nq} K_q^{(n)}=\Z/p$ \textbf{is this true?}, the map
\[
