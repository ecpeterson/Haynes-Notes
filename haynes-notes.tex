...
\begin{cases}0;&\textup{if }i<nq;\\(H^q(X)^{\otimes n})^\pi;&\textup{if }i=nq;\\???;&\textup{if }i>nq\end{cases}
\]
\end{lem}
\begin{proof}
...
.\]
Now $H^*(X^n, F_{n-1} X^n) \cong \widetilde H^* X^{(n)} \cong \widetilde H^*(X)^{\otimes n}$, and this last isomorphism is equivariant with respect to permutations, a fact that is not trivial (and likely to be false with other cohomology theories).  It follows that in the spectral sequence everything is zero below the horizontal line at height $nq$ in the $E_2$ term.  So, % why?
\[
...
Take for example the case $K = K(\Z/2, q)$. By the Hurewicz theorem and universal coefficients theorem: % rework this section so that we can swap out singular cohomology for something else.  this seems to be the key confusing step
\[\widetilde H^i(K; \Z/2)=0\text{ \ for $i<q$, and \ }\widetilde H^q(K; \Z/2)=\Z/2.\]
%%%$\widetilde H^i(K; \Z/2)$ is zero for $i<q$ and is $\Z/2$ when $i=q$.
%\begin{align*}
%\widetilde H^i(K; \Z/2) & = 0, \hspace{5em} i < q \\
%\widetilde H^q(K; \Z/2) & = \Z/2,
%\end{align*}
%by the Hurewicz theorem.  
So $\widetilde H^{nq}(D_\pi K; \Z/2) = \Z/2$.  Moreover the spectral sequence for $\pi \into E\pi_+ \sprod K^{(n)} \xrightarrow{\ p\ } E\pi_+ \sprod_\pi K^{(n)}$ shows that the map
\[
\Z/2 = \widetilde H^{nq} D_\pi K \xrightarrow{\ p^*\,} \widetilde H^{nq} K^{(n)} % this breaks if the action of pi isn't transitive
\]
