...
It's time to draw some pictures.  First, let's see where all these relative attaching maps go.  Here's $\RP^\infty_{-1}$; lines show where the top cell of $\RP^{n-1}_{-1}$ is attached. Here, the unlabeled maps for attaching the even dimensional cells are all $2\iota$ --- they do not descend to any lower skeleta, even stably.
\begin{figure}[h!]
...
\end{thm}
\ConfusedBox{Insert nice picure of these differentials here.}
...
Now as $[\iota]\in {_\text{AH}E}^1_{n-1,0}$ survives to $E^{\rho(n)}$, at which point it supports a nonzero differenial, $[\iota]\in {_\text{EHP}E}^1_{n-1,0}$ must support a nonzero differential $d_r$ for some $r\leq\rho(n)$. This shows that $w_{n-1}=p\iota$ desuspendes at most $\rho(n)-1$ times.
