...
%\newcommand{\OutputUsingTheSquaresAndThomSpaces}{}
...
%\newcommand{\OutputProofOfHopfInvariantOne}{}
%\newcommand{\OutputTheJamesConstruction}{}
%\newcommand{\OutputTheMapsEandHinTheEHPLES}{}
%\newcommand{\OutputWhiteheadProductsAndTheEHPSS}{}
%\newcommand{\OutputTheStableCategory}{}
%\newcommand{\OutputHomotopicalAlgebraAndDuality}{}
%\newcommand{\OutputDualities}{}
%\newcommand{\OutputTheStructureOfStuntedProjectiveSpace}{}
%\newcommand{\OutputMatchingTheEHPSStoAtiyahHirzebruck}{}
%\newcommand{\OutputTheSpaceOfLittleCubes}{}
%\newcommand{\OutputTheBeckerGottliebTransfer}{}
%\newcommand{\OutputTheAdamsConjecture}{}
%\newcommand{\OutputSomeSummandsOfTheStableHomotopyGroupsOfSpheres}{}
%\newcommand{\OutputTheBibliography}{}

\ifx\OutputEVERYTHING\undefined\else
\providecommand{\OutputFrontMatter}{}
\providecommand{\OutputForeword}{}
\providecommand{\OutputIntroductionToVectorFieldsOnSpheres}{}
\providecommand{\OutputCliffordAlgebras}{}
\providecommand{\OutputBuildingThomSpaces}{}
\providecommand{\OutputFactsAboutThomSpaces}{}
\providecommand{\OutputBuildingKandJtheory}{}
\providecommand{\OutputGeometryAndTheSteenrodSquares}{}
\providecommand{\OutputPropertiesOfTheSquares}{}
\providecommand{\OutputDerivingTheUsualAdemRelations}{}
\providecommand{\OutputUsingTheSquaresAndThomSpaces}{}
\providecommand{\OutputStructureOnKtheory}{}
\providecommand{\OutputBuildingKtheoryOperatorsFromRepresentationTheory}{}
\providecommand{\OutputBuildingTheAdamsPowerOperations}{}
\providecommand{\OutputProofOfHopfInvariantOne}{}
\providecommand{\OutputTheJamesConstruction}{}
\providecommand{\OutputTheMapsEandHinTheEHPLES}{}
\providecommand{\OutputWhiteheadProductsAndTheEHPSS}{}
\providecommand{\OutputTheStableCategory}{}
\providecommand{\OutputHomotopicalAlgebraAndDuality}{}
\providecommand{\OutputDualities}{}
\providecommand{\OutputTheStructureOfStuntedProjectiveSpace}{}
\providecommand{\OutputMatchingTheEHPSStoAtiyahHirzebruck}{}
\providecommand{\OutputTheSpaceOfLittleCubes}{}
\providecommand{\OutputTheBeckerGottliebTransfer}{}
\providecommand{\OutputTheAdamsConjecture}{}
\providecommand{\OutputSomeSummandsOfTheStableHomotopyGroupsOfSpheres}{}
\providecommand{\OutputTheBibliography}{}
\fi
...
(x, e_1).
\end{diagram}
So $e_1$ is a right-unit of this multiplication on $S^{n-1}$.  $e_1$ isn't necessarily a left unit, but $v(e_1)$ is a matrix such that $v(e_1)_{i, j}$ is $1$ for $i = j = 0$ and $0$ whenever $i = 0$ or $j = 0$ but not both.  We can therefore correct the situation by replacing $v$ with $v(e_1)^{-1} v$, which will still be a section. % is this actually true?
...
\Bullet (It is claimed that) if $\mu$ is an $H$-space multiplication on $S^{n-1}$, $H(\mu)$ has Hopf invariant one. We'll see that there is only an element of Hopf invariant one when $n=1,2,4,8$.

\Bullet The $J$-homomorphism $J:\pi_kO(n)\to \pi_{n+k}(S^n)$ sends $\alpha:S^k\to O(n)$ to the Hopf construction applied to $S^k\times S^{n-1}\xrightarrow{\alpha\times1}O(n)\times S^{n+1}\to S^{n-1}$.
}
...
& & B.
\end{diagram}
Now by the theorem of Dold from before, the composite of the horizontal arrows contains all the information about the fiber homotopy equivalence, i.e., this equivalence is equivalent to a map $f: E \to S^{n-1}$ which is a homotopy equivalence on each fiber of $E$.  Another way to think about this is as a bundle map % maybe Dold isn't necessary here? figure out what is.
\begin{diagram}[height=2em]
E & \rTo^f & S^{n-1} \\
\dTo & & \dTo \\
B & \rTo & \ptspace, % maybe draw these diagrams differently, since the lower-left S^n doesn't come from B in any real way; it's the fiber over the basepoint in B.
\end{diagram}
which induces a map
\begin{diagram}[height=2em]
T(E) & \rTo & S^n \\
...
If the base $B$ is connected, then from Dold's theorem it follows that $E \oplus \epsilon$ is fiber homotopy trivial. % this sentence is completely vacuous when B is a CW-complex, so who cares about Dold's theorem
...
% maybe this belongs in the next day :)
...
What happens when we descend to $KO(X)$?  First, the equivalence $V \sim V \oplus \epsilon$ gives us $\widetilde{KO}$, % is Grothendieck(Vect) / this relation the same as Vect / this relation?
...
\begin{cor}
If $S(nL) \to \RP^k$ is fiber homotopy trivial, then $a_k$ divides $n$, so there are at most $\rho(n) - 1$ vector fields on $S^{n-1}$. % this should have a proof attached, reminding the reader of the chain of implications
\end{cor}
One of the things we get out of this theorem is that $\widetilde{KO}(\RP^k)$ is finite; this follows from the following theorem: % really i think this and the previous immediate theorems should be moved down below Atiyah's theorem
\begin{thm}[Atiyah]
If $X$ is a finite connected complex, then $\tilde J(X)$ is finite.
\end{thm}
\begin{proof}(Sketch, anyway.)
A vector bundle over $X$ is classified by a map $X \to BO(n)$.  Similarly a sphere bundle should have a similar classifying procedure.  In general, the structure group would have to be taken to be $Homeo(S^{n-1})$, % use the same formula as below, but without the basepoints
...
.\]
Now $H^*(X^n, F_{n-1} X^n) \cong \tilde H^* X^{(n)} \cong \tilde H^*(X)^{\otimes n}$, and this last isomorphism is equivariant with respect to permutations, a fact that is not trivial (and likely to be false with other cohomology theories).  It follows that in the spectral sequence everything is zero below the horizontal line at height $nq$ in the $E_2$ term.  So, % why?
\[
\tilde H^{nq}(D_\pi X) = H^0(B\pi; \{\tilde H^q(X)^{\otimes n}\}) = (\tilde H^q(X)^{\otimes n})^\pi
.\qedhere\]
\end{proof}

This is the key fact that gives the Steenrod operations; all else follows more or less from it.  Take for example the case $K = K(\Z/2, q)$. % rework this section so that we can swap out singular cohomology for something else.  this seems to be the key confusing step
\begin{align*}
...
\[
\Z/2 = \tilde H^{nq} D_\pi K \stackrel{p^*}{\to} \tilde H^{nq} K^{(n)} % this breaks if the action of pi isn't transitive
\]
...
\uTo<k & & \uTo>p \\
S^0 \sprod X & \rTo^\Delta & X^{(2)}, % we should check that k induces the projection map on cohomology
\end{diagram}
...
\end{lem}
\begin{proof}
We can assume $X = K(\pi, p)$, $Y = K(\pi, q)$, $u = \iota_p$, and $v = \iota_q$.  Then the lowest dimensional cohomology of $D_\pi(X \sprod Y)$ is $\Z/2$ in dimension $2(p + q)$.  But $(\delta p)^*(P\iota_p \sprod P\iota_q) = ((p \sprod p)T)^* (\iota_p \sprod \iota_q)$ equals $(\iota_p \sprod \iota_q)^{\sprod 2}$, and $P(\iota_p \sprod \iota_q)$ is defined to be the unique class in $H^{2(p+q)} D_\pi(X \sprod Y)$ for which this holds. % we have two nonzero things living in the same dimension, where there's only one nonzero element.
\end{proof}
...
Last time we got as far as showing $\Sq_x \Sq_y u = \Sq_y \Sq_x u$, where $\Sq_x = \sum_{i \ge 0} x^{-i} \Sq^i$, and $|x| = |y| = 1$, and $x$ and $y$ represent $1$-dimensional classes in $\tilde H^* \RP^\infty$. % what the hell
...
$\tilde H^* X$ is an $\SA$-module, and $\Sq^i x = 0$ if $i > |x|$; such things are called ``unstable $\SA$-modules,'' even though this is bad terminology.  Moreover, $x^2 = \Sq^{|x|} x$, and $\Sq(xy) = \Sq(x) \Sq(y)$; such a thing is called an ``unstable $\SA$-algebra.''  If $\CatOf{Unstable-\SA-Algebras}$ is the category of these, then the functor $\tilde H^* : \CatOf{Spaces} \to \CatOf{GradedVectorSpaces}$ factors as
\begin{diagram}
\tilde H^*: \CatOf{Spaces} & \rTo & \CatOf{Unstable-\SA-Algebras} \\
& \rdTo & \dTo \\
& & \CatOf{GradedVectorSpaces},
\end{diagram}
and $\CatOf{Unstable-\SA-Algebras}$ is the largest algebraic category through which $\tilde H^*$ factors. % what is an algebraic category and why is this the largest?

Let's take another look at the formula $y^m = \sum_{k \ge m} \binom{-k-1}{k-m} s^{t-m} t^{-k}$.  Setting $m=1$ gives \[y = \sum_{k \ge 1} \binom{-k-1}{k-1} s^{k-1} t^{-k}.\]  On the other hand, we know $y = t^{-1} + sy^2$.  Since squaring is a homomorphism in $\Z/2$, we get
\begin{align*}
y & = t^{-1} + sy^2 \\
& = t^{-1} + s(t^{-2} + s^2 y^4) \\
...
\begin{diagram}
H^* B \otimes H^*(C_B E, E) & \rTo^{p^* \otimes 1} & H^*(C_B E) \otimes H^*(C_B E, E) \\
...
\begin{rem}
In the real case, we have $\widetilde{KO}(S^8) = \Z$ and in fact is generated by the tautological Cayley $\mathbb{H}$-line bundle, considered as a 4-dimensional real bundle over $\mathbb{H}P^2 \simeq S^8$. (What happens if we use octonions?) % is this last bit right?  and what about the octonions?
\end{rem}
...
\begin{diagram}
\mathrm{Vect}(X) & \rTo^{\Lambda_t} & KO(X) \llbracket t \rrbracket^\times \\
\dTo & \ruTo_{\lambda_t} \\
KO(X),
\end{diagram}
i.e., for $[E], [F] \in KO(X)$,
...
\begin{diagram}
\prod_n \mathrm{Vect}_n(X) & \rTo^\theta & K(X) \\
\dTo & \ruTo_\theta \\
K(X).
\end{diagram}
...
\begin{diagram}
\coprod_n \mathrm{Rep}_n G & \rTo^\varphi & R(G) \\
...
Instead, operations in $KO$ can be obtained from the complex case: use $O(n)$, and for a compact Lie group $G$ use $RO(G)$ the ring of virtual real representations.  Complexification of real representations gives a map $c: RO(G) \to R(G)$ which is monic, and the diagram
\begin{diagram}
RO(G) & \rInto^c & R(G) \\
\dInto<\chi & & \dInto>\chi \\
\{\hbox{$\R$-class functions}\} & \rInto & \{\hbox{$\C$-class functions}\}
\end{diagram}
commutes.  Moreover, there is the inclusion $i: O(n) \into U(n)$ which induces $i^*: RU(n) \to RO(n)$.  The real Adams operations come from additive $RO$-sequences $\{\psi^k_{\R, n}, n \ge 0\}$:
\begin{diagram}
& & RO(O(n)) & \ni \psi^k_{\R, n} & = s_k(\Lambda^1, \ldots, \Lambda^k) \\
& & \dInto & \ldMapsto(2,3) \\
R(U(n)) & \rTo^{i^*} & R(O(n)) \\
...
\begin{lem}
For $p$ prime, $\psi^p(x) \equiv x^p \pmod{p}$ in $K(X)$ (so think of $\psi^p$ as an improvement of the Frobenius ring homomorphism).
\end{lem}
\begin{proof}
Suppose $x$ is a vector bundle $E$.  Then we saw last time $\psi^p(E) = s_p(\Lambda^1 E, \ldots, \Lambda^p E)$.  Now \[s_p(\sigma_1, \ldots, \sigma_n) = \sum_{i=1}^n z_i^p = \left( \sum z_i \right)^p + p \cdot v(\sigma_1, \ldots, \sigma_n),\] that is, $\sum_{i=1}^n z_i^p - \left( \sum_i z_i \right)^p$ is a polynomial all of whose coefficients are p-divisible and which is certainly symmetric in the $z_i$.  So it can be written as $p \cdot r(\sigma_1, \ldots, \sigma_n)$.  Well, this looks pretty good; $\sum_{i=1}^n z_i = \sigma_1$, so
\begin{align*}
\psi^p(E) & = s_p(\Lambda^1 E, \ldots, \Lambda^n E) \\
& = (\Lambda^1 E)^p + p \cdot r(\Lambda^1 E, \ldots, \Lambda^n E) \\
& \equiv E^p \pmod p, \\
\psi^p(E - F) & = \psi^p(E) - \psi^p(F) \\
& = E^p + p \cdot r(E) - F^p - p \cdot r(F) \\
& \equiv E^p - F^p \pmod{p} \\
& \equiv (E - F)^p \pmod{p}. \qedhere
\end{align*}
\end{proof}
...
\begin{diagram}
0 & \rTo & \tilde K(X) & \rTo & K(X) & \rTo & \Z & \rTo 0,
\end{diagram}
...
\begin{diagram}
0 & \rTo & \tilde K(X) & \rTo & \tilde K(X) \oplus \Z & \rTo & \Z & \rTo 0.
\end{diagram}
Next tensor the first two terms of this with the same sequence for another space Y; the cokernel is easily computed as
\begin{diagram}
0 & \rTo & \tilde K(X) \otimes \tilde K(Y) & \rTo & (\tilde K(X) \oplus \Z) \otimes (\tilde K(Y) \oplus \Z) & \rTo^\alpha & \tilde K(X) \oplus \tilde K(Y) \oplus \Z & \rTo 0.
\end{diagram}
...
(t, x) & \rMapsto & [t, (x, *)] & \rMapsto & \left\{ \begin{array}{cc}2t(x, *) & t \le \frac{1}{2} \\ 2-2t(x, *) & t \ge \frac{1}{2} \end{array} \right\} & \rMapsto & \begin{array}{cc} (2t, x) & t \le \frac{1}{2} \\ * & t \ge \frac{1}{2}, \end{array}
\end{diagram}
...
Today we start looking at consequences of the vector field problem.  First, remember the group $J(X)$.  This was the quotient of $KO(X)$ by the equivalence generated on the level of bundles by $V \simeq_J W$ if and only if $S(V \oplus n \varepsilon) \simeq_{f.h.e.} S(W \oplus n \varepsilon)$.  An immediate consequence of the last few days' work is
\begin{thm}
The surjection $\widetilde{KO}(\RP^m) \onto \tilde J(\RP^m)$ is actually an isomorphism.
\end{thm}
\begin{proof}
$\widetilde{KO}(\RP^m)$ is generated by $\bundle{L} - 1$.  Now $n(\bundle{L} - 1) \mapsto 0$ means $n \bundle{L}$ is stably fiber homotopy trivial; earlier we showed that this implise a (stable) splitting
\begin{diagram}
\RP^{m+n}_n & \rTo & S^n \\
\uTo & \ruTo_\simeq \\
S^n.
\end{diagram}
But now we know that this implies that $\nu(n) > \varphi(m)$, so $n \ge 2^{\varphi(m)}$, so $n(\bundle{L} - 1)$ was already zero in $\widetilde{KO}(\RP^m)$.\footnote{Homework: compute $\tilde J(S^n)$.}
\end{proof}
...
\dTo<{\Suspend(\hbox{collapsing})} & \ldTo{\hat h_m} \\
\Suspend X^{(m)}.
\end{diagram}
...
g \otimes 1 + 1 \otimes g & \rMapsto & h(g \otimes 1 + 1 \otimes g) & \lEqualto u_2 \otimes 1 + 2 u_1 \otimes u_1 + 1 \otimes u_2.
\end{diagram}
...
OK, so now we'll go back to the EHP sequence, and conclude this look at the map $h$ by seeing what we get from the existence of elements of Hopf invariant 1.  Last time we found an element of Hopf invariant one $\nu: S^7 \to S^4$ which came from the Hopf construction on quaternion multiplication $S^3 \times S^3 \to S^3$.  In the EHP sequence this appears as the $\pi_6$ level of $S^3 \to \Loops S^4 \stackrel{h}{\to} \Loops S^7$.  We can fit $\nu$ into this picture by
\begin{diagram}
S^3 & \rTo & \Loops S^4 \\
& \ruTo(2,4)^{\Loops \nu} & \uTo & \rdTo(2,4)^h \\
& & S^6 \\
& \ldTo^\alpha & & \rdTo \\
\Loops S^7 & & \rTo & & \Loops S^7,
\end{diagram}
where the maps $S^6 \to \Loops S^7$ and $\Loops S^7 \to \Loops S^7$ we claim are homotopic to $\pm 1$.  It suffices to show this in cohomology -- but we know, since Hopf-Hopf = James-Hopf, that $\Loops S^7 \stackrel{\Loops \nu} \Loops S^6 \stackrel{h}{\to} \Loops S^7$ is $\pm 1$ in $\pi_6$ and so in $H^6$ by the Hurewicz theorem.  And since in $\Loops S^7$ we have generators $u_i \in H^{6i}$ with $u_1^i = i! u_i$ we get $u_i \mapsto u_i$ (i.e., there is no torsion around).

This is amazing: it means that $\Loops S^4$ splits; that is,
\begin{diagram}
S^3 & \rTo & \Loops S^4 & \pile{\lDashto \\ \rTo_h} & \Loops S^7,\hbox{ so} \\
S^3 \times \Loops S^7 & \rTo^{e \times \Loops \nu} & \Loops S^4 \times \Loops S^4 & \rTo^\mu & \Loops S^4
\end{diagram}
is a homotopy equivalence!  Similarly $\eta \in \pi_3 S^2$, $\sigma \in \pi_{15} S^8$ with Hopf invariant 1 provide splittings $S^1 \times \Loops S^3 \stackrel{\simeq}{\to} \Loops S^2$ and $S^7 \times \Loops S^{15} \stackrel{\simeq}{\to} \Loops S^8$, and this \emph{never} happens again, as it is equivalent to the existence of elements of Hopf invariant one.

What does this map look like on homotopy groups?  Well, when there is an H-map $\mu: X \times X \to X$ and $X$ is connected, you get a homomorphism
\begin{diagram}
\pi_* X \times \pi_* X & \rTo & \pi_* X \\
(\alpha, 0) + (\beta, 0) & \rMapsto & \alpha + \beta
\end{diagram}
(note that it follows that $\pi_1$ is abelian!).  So the isomorphism for $S^3 \times \Loops S^7 \simeq \Loops S^4$ is
\begin{diagram}
\pi_n S^3 \times \pi_{n+1} S^7 & \rTo^{\cong} & \pi_{n+1} S^4 \\
(\alpha, \beta) & \rMapsto & e \alpha + \nu \circ \beta.
\end{diagram}
...
\begin{enumerate}
\item We can truncate the sequence of triangles anywhere we want, obtaining a spectral sequence, compatible in some sense with the first, that converges to the homotopy of a finite sphere.
\item The second lesson concerns how an element of $\pi_* S^n$ is recorded in the truncated spectral sequence of (1) (and therefore in the big spectral sequence if you take $n$ large enough).  Since everything is recorded in terms of its (James) Hopf invariant, and this is the obstruction to desuspending an element of $\pi_* \Loops^n S^n$, the recipe is
\begin{enumerate}
...
As a final remark, note that now we can talk about $T(\mathrm{virtual bundle})$; for example, $T(V - \varepsilon_n) = \Suspend^{-n} T(V)$.
...
\begin{thm}
/If $Y \simeq S^n - X$ then $\hat f$ is a duality; $DX \simeq \Suspend^{1-n} Y$.  The proof consists of showing that $\hat f/: \tilde H_i \Suspend^{1-n} Y \stackrel{\cong}{\to} H^i X$ is an isomorphism for $i \in \Z$.
\end{thm}
\begin{rem}
...
By th eway, if you don't like these somewhat ethereal spaces, you can use James Periodicity: let $a$ be the periodicity of $\bundle{L}$ in $\tilde J(\RP^{t+b-1})$.  Then $T((ja-b)\bundle{L}) \simeq \Suspend^{ja} T(-b \bundle{L})$; for $j$ big enough, $T((ja-b)\bundle{L})$ is an actual stunted projective space!
\begin{rem}
...
& & \vdots & & \vdots,
\end{diagram}
along with the map $j_{\nu(n)}: S^{n-2} \to S^{n - \rho(n) - 1}$.  Now taking $[S^{n-2}, -]$ on this diagram is exactly what we nee dto get a Mayer-Vietoris sequence; two long exact sequences with maps between them which are isomorphisms in every third position.  The two composites
\begin{diagram}
...
On Friday we constructed a candidate, an Atiyah-Hirzebruch spectral sequence for the attaching maps on $\RP^\infty$: $\tilde H^*(\RP^\infty_+; \pi_*^S)$ which we hope converges to $\pi_*^S \RP^\infty_+$, whose $E^1$-term is INKSCAPE
...
\end{diagram}
If $u \in \tilde H^n(X^\xi)$ is the Thom class of $\xi$ define the Euler class $e_\xi$ as the image under pullback by $\zeta$:
\begin{diagram}
\tilde H^n X^\xi & \rTo^{\zeta^*} & \tilde H^n(X^0) & \rEqualto & \tilde H^n(X) \\
u_\xi & \rMapsto & e_\xi.
\end{diagram}
...
\end{diagram}
Let $\sigma \in \tilde H^n S^n$ be a generator; think of it as the Thom class of $\R^n$ over $\ptspace$.  The diagram
\begin{diagram}
\nu & \rTo & \nu \oplus \tau & \rTo & \R^n \\
\dTo & & \dTo & & \dTo \\
...
Now let $\Sigma_{n-1}$ act on $F(V)$ by fixing the last element; then there is an $n$-sheeted cover $\tilde L(V) = F(V)/\Sigma_{n-1} \stackrel{q}{\to} L(V)$.  Moreover, there is a $2$-plane bundle $\zeta$ over $\tilde L(V)$ defined fiberwise by $\zeta_{(\{v_1, \ldots, v_{n-1}\}, v_n)} = v_n$.  Now from the lemma at the beginning of this lecture, it suffices to check the Adams conjecture on $p^* \xi$.  But by the formula,
\begin{align*}
...
\ifx\OutputSomeSummandsOfTheStableHomotopyGroupsOfSpheres\undefined\else
...
\begin{diagram}
& 0 & 1 & 2 & 3 & 4 & 5 & 6 & 7 \\
\pi_* J & \Z_{(2)} & \Z/2 & \Z/2 & \Z/8 & 0 & 0 & 0 & \Z/16 \\
& \dTo & \dTo & \dTo & \dTo & \dTo & & & \\
\pi_* bo & \Z_{(2)} & \Z/2 & \Z/2 & 0 & \Z_{(2)} & 0 & 0 & 0 \\
& \dTo>0 & \dTo>0 & \dTo>0 & & \dTo>{\cdot 9 - 1} & & & \\
\pi_* bo \langle 4, \ldots, \infty \rangle & 0 & 0 & 0 & 0 & \Z_{(2)} & 0 & 0 & 0.
\end{diagram}
\begin{diagram}[width=3.5em]
& 8 & 9 & 10 & 11 & 12 & 13 & 14 & 15 & 16 \\
\pi_* J & \Z/2 & (\Z/2)^2 & \Z/2 & \Z/8 & 0 & 0 & 0 & \Z/32 & \Z/2 \\
& \dTo>0 & \dTo \\
\pi_* bo & \Z_{(2)} & \Z/2 & \Z/2 & 0 & \Z_{(2)} & 0 & 0 & 0 & \Z \\
& \dTo>{\cdot 81 - 1} & \dTo>0 & \dTo>0 & & \dTo>{\cdot 9^3 - 1} & & & & \dTo>{\cdot 9^4 - 1} \\
\pi_* bo \langle 4, \ldots, \infty \rangle & \Z_{(2)} & \Z/2 & \Z/2 & 0 & \Z_{(2)} & 0 & 0 & 0 & \Z.
\end{diagram}
