...
\begin{center}\fbox{\begin{minipage}{.95\textwidth}
#1
\end{minipage}}
\end{center}
}
...
\Bullet With PID \textup{\textbf{(should this say `field'?)}} coefficients, there are isomorphisms:
\begin{alignat*}{2}
...
A simple argument using the Serre spectral sequence of the homotopy fibration $F \to \Loops S^{n+1} \to \Loops S^{2n+1}$ (\textbf{draw it})
...
\end{thm}
So for $n$ even, the $2$-local homotopy groups are the same; we have no idea about the unlocalised homotopy groups, but for present purposes we don't care: we get the same EHP sequeneces for $n$ even, but now localized at 2.
\end{itemize}
...
\caption{\small The complex $K$ used to define the stable Hopf invariant.}
\end{wrapfigure}
...
\end{lem}
\begin{proof}
We'll calculate $h(f)=\pm H(f)$ for $f\in\pi_{2n+1} (S^{n+1})$. First take the case $n$ is even.  Then $H(f)$ is calculated by squaring an odd dimensional cohomology class, so that $H(f) = 0$. 
%To calculate $h(f)$, we use the previous claim. It is enough to prove that $H_{2n}(\hat f)$ is the zero map. 

Now let $g$ generate $H_{2n}(S^{2n})$, and write $u_2$ as usual for the generator of $H_{2n}(\Omega S^{n+1})$. By the claim, $\hat f_*(g)=h(f)\cdot u_2$ (up to sign). Now $f_*$ is a coalgebra map, and we have computed the Hopf algebra structure on $H_*(\Omega S^{n+1})=\mathrm{T}[u_n]$, so there is a compatibility:
\begin{diagram}
g & \rMapsto^{\hat f_*} & h(f) u_{2n} \\
\dMapsto & &\rdMapsto>\Delta \\
g \otimes 1 + 1 \otimes g & \rMapsto & h(f)(u_2 \otimes 1 + 1 \otimes u_2) & \lEqualto u_2 \otimes 1 + 2 u_1 \otimes u_1 + 1 \otimes u_2.
\end{diagram}
\[\xymatrix{
g\ar@{|->}[r]_{\hat f_*}\ar@{|->}[d]_{\Delta}&h(f)\cdot u_{2n}\ar@{|->}[dr]^{\Delta}\\
g\otimes1+1\otimes g\ar@{|->}[r]^{\hat f_*\otimes\hat f_*\qquad\ \ }&\ar@{=}[r]h(f)\cdot(u_{2n}\otimes1+1\otimes u_{2n})&h(f)\cdot(u_{2n}\otimes1+2u_1\otimes u_1+1\otimes u_{2n})
}\]
But this equality can hold only if $h(f) = 0$, so we are done when $n$ is even.
...
\begin{diagram}[height=2em]
S^{2n+1} & \rTo^f & S^{n+1} & \rTo & C(f) & \rTo & S^{2n+2} \\
\dEqualto & & \uTo>\beta & & \uTo & & \dEqualto \\
S^{2n+1} & \rTo^{\Suspend \hat f} & \Suspend \Loops S^{n+1} & \rTo & \Suspend C(\hat f) & \rTo & S^{2n+2} & \rTo^{\Suspend^2 \hat f} & \Suspend \Loops S^{n+1} \\
& & 0 & \lTo & \Z_h & \lTo & \Z & \lTo^{\cdot h} & \Z \makebox[0cm][l]{\ \ in $H^{2n+2}$}, \\
& & x & \lMapsto & x, y & \lMapsto & y \makebox[0cm][l]{$\ \in H^{2n+2}(S^{2n+2})$} \\
