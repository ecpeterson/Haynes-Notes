...
From here on, for all $r$, let $K_r$ denote $K(\Z_p,r)$ and $\widetilde H^r$ denote $\widetilde H^r(\textup{---};\Z_p)$, for some chosen prime $p$.  By the Hurewicz theorem and universal coefficients theorem: % rework this lecture so that we can swap out singular cohomology for something else.  this seems to be the key confusing step
...
In any case, we want to understanding the stable relative attaching maps for $\RP^\infty$ using the standard cell structure, with one cell in each dimension. For example, in the following diagram, we may ask for the largest value of $j$, stably, and may ask what the compression $ba$ of the stable attaching map onto the top cell is, as an element of $\pi^S_j$.
\[\xymatrix@!0@C=3.5cm@R=1.4cm{
...
Thus:
\begin{thm}
The stable relative attaching map for the $(n-1)$-cell of $\RP^{n-1}_{-N}$ can be taken to be $j_{\nu(n)}$; that is, stably, there is some $\overline \pi$ so that
\[\xymatrix@!0@C=3.5cm@R=1.4cm{
\RP^{n-\rho(n)-2}_{-N}\ar@{^{(}->}[r]^\imath &\ar[d] \RP^{n-\rho(n)-2}_{-N}\ar|{\ \cdot\ \cdot\ \cdot\ }@{^{(}->}[r]& \RP^{n-2}_{-N} \ar@{^{(}->}[r] & \RP^{n-1}_{-N}\\
&\makebox[0cm][r]{$C(\imath)=$\,}S^{n-\rho(n)-1}&\ar@{^{(}->}[r]\ar[ul]_{\overline\pi}\ar@{-->}[ull]^(.75){\text{\large$\nexists$}}S^{n-2}\ar@{^{(}->}[r]\ar[u]^\pi\ar[l]^{j_{\nu(n)}}&D^{n-1}\ar[u]
}\]
\end{thm}

It's time to draw some pictures.  First, let's see where all these relative attaching maps go.  Here's $\RP^\infty_{-1}$; lines show where the top cell of $\RP^{n-1}_{-1}$ is attached. Here, the unlabeled maps for attaching the even dimensional cells are all $2\iota$ --- they do not descend to any lower skeleta stably.
\begin{figure}[h!]
...
\[\xymatrix{
\cdots\ar@{^{(}->}[r]&\RP^{k-1}_{-1}\ar@{^{(}->}[r]\ar[d]&\RP^{k}_{-1}\ar@{^{(}->}[r]\ar[d]&\RP^{k+1}_{-1}\ar@{^{(}->}[r]\ar[d]&\cdots\\
&S^{k-1}\ar@{~>}[ul]&S^k\ar@{~>}[ul]&S^{k+1}\ar@{~>}[ul]
}\]
Here the vertical arrows are the collapse maps,
% onto the top cells of the $\RP^k_{-1}$, 
and the wavy arrows, which are wavy to indicate that they are in fact maps $\Sigma^{-1}S^k\to\RP^{k-1}_{-1}$, are the attaching maps. We can apply $\pi_*^S$ to obtain an exact couple:
\ResumeHere
\begin{diagram}[height=2em]
