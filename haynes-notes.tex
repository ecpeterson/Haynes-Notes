...
Where we have defined, for $t+b\geq1$,
...
\mathrm{Sk}_{k-j-1} (X) \ar@{^{(}->}[r] & \mathrm{Sk}_{k-j} (X) \ar@{^{(}->}[r] & \mathrm{Sk}_k (X) \ar@{^{(}->}[r] & \mathrm{Sk}_{k+1} (X)\\
...
\mathrm{Sk}_{k-j+n-1} (\Sigma^nX) \ar@{^{(}->}[r]^\imath &\ar@{~>}[u]^b \mathrm{Sk}_{k-j+n} (\Sigma^nX) \ar@{^{(}->}[r] & \mathrm{Sk}_{k+n} (\Sigma^nX) \ar@{^{(}->}[r] & \mathrm{Sk}_{k+n+1} (\Sigma^nX)\\
...
\ResumeHere
In any case, we want to understanding the stable relative attaching maps for $\RP$; for example, in the following diagram (formatted a little differently to those before) we ask
\[\xymatrix@!0@C=3.5cm@R=1.4cm{
\RP^{k-j-1}\ar@{^{(}->}[r] & \RP^{k-j}\ar@{^{(}->}[r] & \RP^k \ar@{^{(}->}[r] & \RP^{k+1}\\
&S^k\ar@{^{(}->}[r]\ar[u]^{a'}\ar@<.6ex>[ur]_r\ar@{-->}[ul]^{\text{\large$\nexists$}}&\bigvee S^k\ar@{^{(}->}[r]\ar[u]&\bigvee D^{k+1}\ar[u]
}\]
\begin{diagram}[height=2em]
