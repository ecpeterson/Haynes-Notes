...
\end{diagram}
To explain this pictorially, we represent $X$, $Y$ and $Z$ each as a single point, and draw $X\ast Y$ as:
\begin{center}
\begin{tikzpicture}
\path (1.25-6,0) node[below] {$C_+X$};
\path (-6,0) node[below left] {$X$};
\HopfDiagram{}{}{}{-6}{0}{BottomBar}
\path (-3,.75) node[left] {$C_-Y$};
\path (-3,0) node[below left] {$Y$};
\HopfDiagram{}{}{}{-3}{0}{LeftBar}
\HopfDiagram{}{}{}{0}{0}{}
\path (0,.75) node[left] {$X\times C_-Y$};
\path (1.25,0) node[below] {$C_+X\times Y$};
\path (0,0) node[below left] {$X\times Y$};
\end{tikzpicture}
\end{center}
For brevity, write $j$ for $H(\mu)$; since the composite $X \ast Y \stackrel{j}{\to} \Suspend Z \to Cj$ is null-homotopic, it extends to a  map $k:C(X\ast Y)\to C(j)$
In fact, we obtain a map $k$ for every nullhomotopy of this composite, and the obvious choice of nullhomotopy gives rise to the obvious map $k:C(X\ast Y)\to C(j)=\Sigma Z\cup_j C(X\ast Y)$.
In particular, this $k$ is a relative homeomorphism away from $X\ast Y\subset C(X\ast Y)$.
...
\HopfDiagram{}{}{WhiteBeard}{12.5}{3}{cone}

\draw[->] (3.75,.5) -- (5.25,.5);
\draw[->] (9.25,.5) -- (10.75,.5);
\draw[->] (3,3.5) -- (6.25,3.5);
\draw[->] (8.25,3.5) -- (11.75,3.5);
\draw[->] (2.25,2.75) -- (2.25,1.25);
\draw[->] (7.25,2.25) -- (7.25,1.25);
\draw[->] (7.25+5.5,2.25) -- (7.25+5.5,1.25);

\end{tikzpicture}
\end{center}
\begin{figure}[h!]\label{John}
\centering\includegraphics[width=0.4\textwidth]{figures/20.pdf}
\caption{\small Diagram of the cone of a join.}
\end{figure}
Finally, note that $C(X \ast Y) = CX \times CY$, as indicated by figure \ref{John}.  

We are particularly interested in the case where $X=Y=Z=S^{n-1}$:
\[\xymatrix{
S^{n-1} \times \{x_0\}\ar[d]
...
\begin{diagram}[height=2em]
(e^n \times e^n, S^{2n-1}) & \rTo^k_{\text{rel.homeo.}} & (Cj, S^n) & \lTo & Cj \\
\dTo & & \dTo & & \dTo>\Delta \\
