...
In the special case that $E' \to B'$ is $\R^n \to \ptspace$, we get that $T(V \oplus n \epsilon) = T(V) \sprod S^n = \Suspend^n T(V)$. Note that if $V$, $W$ are vector bundles over $B$, it is \textbf{not} the case that $T(V\oplus W)=T(V)\wedge T(W)$. However, we have a pullback diagram (drawn on the left) which induces a map of Thom spaces. This map commutes with the inclusion of the copies of $S^{n+m}$ (on the right, where $\dim_*V=n$ and $\dim_*W=m$):
\[\xymatrix{
V\oplus V'\ar[d]\ar[r]&V\times V'\ar[d]\\
B\ar[r]^\Delta&B\times B
}\qquad\qquad
\xymatrix{
T(V\oplus V'\downarrow B)\ar[r]&T(V\times V'\downarrow B\times B)
\ar@{=}[r]&T(V\downarrow B)\wedge T(V'\downarrow B)\\
&S^{n+n'}\ar[ul]\ar[u]\ar@{=}[r]&S^n\wedge S^{n'}\ar[u]
}\]
...
So $\widetilde H^{nq}D_\pi K_q= \Z/p$.  Moreover, as $\pi$ can only act trivially on $\widetilde H^{nq} K_q^{(n)}=\Z/p$ \textbf{(is this true if $p\neq2$, or should we just set $p=2$ already?)}, the map
\[
...
%Using the correspondence $\widetilde H^nX=[X,K(n,\Z/2)]$, and applying
...
\Bullet The Steenrod algebra $\SA$ is the tensor algebra on symbols $\Sq^i$, modulo the Adem relations. $\widetilde H^*X$ is an ``unstable $\SA$-module''.
...
There's another approach to this sort of calculation with Thom spaces and squares.  Remember that for an $S^{n-1}$-bundle $E$, we had an inclusion into the fiberwise mapping cone $C_B E$, a disk bundle:
\begin{diagram}
S^{n-1} & \rTo & E & \rInto & C_B E & \lTo & D^n \\
& & & \rdTo(1, 1) \ldTo(1, 1) B \\
\end{diagram}
...
\item $w_k = 0$ for $k > n$.
\item Whitney sum formula:\footnote{Whitney says this is the hardest theorem he ever proved, but he had the wrong definition of Stiefel-Whitney classes.} In short, $w(E'\, \widehat\ast\, E'')=w(E')\wedge w(E'')$ for sphere bundles $E'\downarrow B'$ and $E''\downarrow B''$, and thus $w(V'\oplus V'')=w(V')\wedge w(V'')$ for vector bundles $V',V''$. To make sense of this formula, we must note that $T(E'\, \widehat\ast\, E'')=T(E')\wedge T(E'')$ and $T(V'\oplus V'')=T(V')\wedge T(V'')$.
...
\begin{proof}[Proof of Whitney sum formula]
 Let $E' \downarrow B'$ be an $S^{n-1}$-bundle and $E'' \downarrow B''$ be a $S^{q-1}$-bundle; the fiberwise join $E'\, \widehat\ast\, E'' = E / B' \times B'' = B$ is an $S^{p+q-1}$-bundle.  Now, $T(E) = T(E') \sprod T(E'')$, and the Thom classes match up because the fibers do, % this doesn't make sense without characterizing the Thom class as the thing that restricts to the generator on each fiber
