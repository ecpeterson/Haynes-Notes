...
Moreover, it happens\footnote{To see this, one argues as follows. View $\bundle{L}$ as a subbundle of $\RP^{k-1}\times \R^k$, where $\RP^{k-1}$ is viewed as the set of lines $\R\{x\}$ in $\R^k$, for nonzero $x\in\R^k$. Then we can produce a bundle isomorphism $\Hom(\bundle{L},\bundle{L}^\perp)\to\tau(\RP^{k-1})$ by sending $h:\R\{x\}\to \R\{x\}^\perp$ to the germ at $0$ of the path $t\mapsto \R\{x+t\cdot h(x)\}$ in $\RP^{k-1}$. Noting that $\Hom(\bundle L,\bundle L)$ is a line bundle with a nonvanishing section, it is trivial, so $\tau(\RP^{k-1})+\R=\Hom(\bundle L,\bundle L^\perp\oplus\bundle L)=\Hom(\bundle L,\R^k)=\Hom(\bundle L,\R)^k=\bundle L^k$.}\todo{I find this footnote very hard to read.} that $\tau(\RP^{k-1}) + \varepsilon = k \bundle{L}$.
%\begin{enumerate}
