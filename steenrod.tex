...
\begin{center}
\begin{jointikzcd}[row sep = small]
\diagram
    X^{(n)}\rar["u^{\sprod n}"]\ar[dd,"i"] & K_q^{(n)}\ar[dd,"i"]\rar["\imath_q^{\sprod n}"] & K_{nq}\\\\
    D_\pi X\rar["D_\pi u"]& D_\pi K_q\ar[uur,dashed,"P_\pi\imath_q"']
%
\diagram
    \\\tikzintertext{which induces:}\\
%
\diagram
    \Htwee^{nq}X^{(n)} & \Htwee^{nq}K_q^{(n)}\lar["(u^{\sprod n})^*"'] &
    \Htwee^{nq}K_{nq}\mathrlap{{}\ni\imath_{nq}}\hskip-1ex \ar[ddl,dashed,"(P_\pi\imath_q)^*"]\lar["(\imath_q^{\sprod n})^*"']\\\\
    %
    \Htwee^{nq}D_\pi X\ar[uu,"i^*"] & \Htwee^{nq}D_\pi K_q\lar["(D_\pi u)^*"']\ar[uu,"i^*"',"\cong"]
\end{jointikzcd}%
$\phantom{{}\ni\imath_{nq}}$
\end{center}
%
...
%
\diagram \\\tikzintertext{which induces}\\
%
\diagram
