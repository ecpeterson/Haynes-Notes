...
\begin{ctikzcd}
\mathllap{\RP^{k-1}\times S^{n-1}={}}S(n\epsilon) \rar["\shat"]\dar & S(nL)\mathrlap{{}=S^{k-1}\times_{\Z/2}S^{n-1}}\dar\\
...
\end{lem}
\begin{proof}
Define $\shat: \R^k \times S^{n-1} \to \R^k \times S^{n-1}$ by $\shat(x, v) = (x, s(v) x)$.  It is straightforward to see $|s(v) x| = 1$ if $|x| = 1$, so $\shat$ maps $S^{k-1} \times S^{n-1} \to S^{k-1} \times S^{n-1}$.  Quotienting the target by $\Z_2$, we have \[\shat(-x, v) = (-x, -s(v)x) = (x, s(v) x) = \shat(x, v),\] so it descends to a quotient on the source: $\shat: \RP^{k-1} \times S^{n-1} = S(n \epsilon) \to S^{k-1} \times_{\Z_2} S^{n-1} = S(nL)$.  Finally, $s(v)e_1 = v$ (as $s$ is a section), so $\shat(e_1, v) = (e_1, v)$.
\end{proof}
...
%
\begin{ctikzcd}
V\oplus W \rar\dar & V\times W\dar  &   T((V\oplus W)\downarrow B) \rar& T((V\times W)\downarrow(B\times B) \rar[equal] & T((V\downarrow B)\wedge T(W\downarrow B))\\
B \rar["\Delta"]   & B\times B      &                                  & S^{n+m}    \ular\uar               \rar[equal] & S^n\wedge S^m\uar
\end{ctikzcd}
...
E \rar["f"]\ar[dd] & S^{n-1}\ar[dd] & & T(E)\rar & S^n\mathrlap{{}=T(S^{n-1}\downarrow*)}\hskip-1ex\\
&& \smash{\hskip-1.5em\textup{which induces a map on Thom spaces:}\hskip-1.5em}\\
B \rar & \ast & & S^n\ar[uu]\ar[uur,"\simeq"'] &
\end{tikzcd}%
$\phantom{{}=T(S^{n-1}\downarrow*)}$
% Because we used \mathrlap asymmetrically, in order to get the diagram to center correctly, we need to put back the missing horizontal space using \phantom.
\end{center}
...
\begin{ctikzcd}
E \rar[hook]\dar & X\times \R^N\dlar\\
X
\end{ctikzcd}
...
E|_{U_i}\drar["p"']\ar[rr,"f_i"] && U_i\times \R^n \dlar["\pi"]\\
&U_i&
\end{ctikzcd}
