...
E \rar["f"]\ar[dd] & S^{n-1}\ar[dd] & & T(E)\rar & S^n\mathrlap{{}=T(S^{n-1}\downarrow*)}\\
&& \hskip-1.5em\textup{which induces a map on Thom spaces:}\hskip-1.5em\\
B \rar & \ast & & S^n\ar[uu]\ar[uur,"\simeq"'] &
\end{tikzcd}
$\hskip-2em\phantom{S^n=T(S^{n-1}\downarrow*)}$
