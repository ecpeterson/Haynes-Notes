...
\begin{ctikzcd}
\edgellap{\RP^{k-1}\times S^{n-1}=}S(n\epsilon) \rar["\shat"]\dar & S(nL) \edgerlap{{}=S^{k-1}\times_{\Z/2}S^{n-1}}\dar\\
...
%
\begin{cjointikzcd}[row sep = tiny]
\diagram
    E \rar["f"]\ar[dd] & S^{n-1}\ar[dd] \\\\
    B \rar & \ast 
%
\diagram \\\tikzintertext{which induces a map on Thom spaces:}\\
%
\diagram
    T(E)\rar & S^n\edgerlap{=T(S^{n-1}\downarrow*)}\\\\
    S^n\ar[uu]\ar[uur,"\simeq"']
\end{cjointikzcd}
%
